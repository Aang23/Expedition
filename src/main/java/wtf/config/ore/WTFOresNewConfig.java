	...
	public static void loadConfig() throws Exception {
		...
		for (String oreGenString : oreSet){
			...
			if (oreGenString.length() > 1){
				...
				if (Objects.equals(oreString, " wtfcore:stone0DecoStatic@2")){
				...
				if (oreString.toLowerCase().contains("wtfblocktype")){
					try {
						...
						StringBuilder gentypestring = new StringBuilder();
						for (Modifier type : types){
							gentypestring.append("WTFBlockType:").append(type.toString()).append(", ");
						}
				...
				boolean denseBlock = config.get(category, "4 Use dense versions of this ore block", modifier == null && preset.denseBlock).getBoolean();
				...
				if (!Objects.equals(biomeModTags[0], "") && biomeModTags.length > 0){
				...
				if (!Objects.equals(reqBiomes[0], "") && reqBiomes.length > 0){
	...
	public static OreGenAbstract getGenerator (String oreString, OreDefReg preset, Configuration config, String genString, IBlockState oreState, int[] genRange, int[] orePerChunk, boolean denseBlock) throws Exception{
		...
		try {
			...
			for (GENTYPE type : types){
				gentypestring.append(type.toString()).append(", ");
			}
