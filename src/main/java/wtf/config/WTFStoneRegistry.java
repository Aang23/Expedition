...
public class WTFStoneRegistry extends AbstractConfig {
	...
	public static void loadStoneReg() throws Exception{
		...
		TreeSet<String> stoneTreeSet = new TreeSet<>(Arrays.asList(stoneset));
		...
		for (String stateString : stoneTreeSet){
			...
			if (state != null){
				...
				String locBlockstate = config.get(stateString, "BlockState resource location", defBlockStateLocations.get(state) == null ? state.getBlock().getRegistryName().toString() : defBlockStateLocations.get(state)).getString();
				String locTexture = config.get(stateString, "Stone texture resource location", defTextureLocations.get(state) == null ? "null" : defTextureLocations.get(state)).getString();
