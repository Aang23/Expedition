...
public class DungeonSpeleothemGrotto extends AbstractDungeonType{
	...
	public void generateCeilingAddons(GeneratorMethods gen, Random random, BlockPos pos, float depth) {
		if (simplex.get2DRandom(gen.getWorld(), pos) < 0.5){
			gen.genSpeleothem(pos, random.nextInt(5), depth, BiomeDictionary.getTypes(gen.getWorld().getBiome(pos)).contains(Type.SNOWY));
		}
	}
	@Override
	public void generateFloorAddons(GeneratorMethods gen, Random random, BlockPos pos, float depth) {
		if (simplex.get2DRandom(gen.getWorld(), pos) < 0.5){
			gen.genSpeleothem(pos, random.nextInt(3), depth, BiomeDictionary.getTypes(gen.getWorld().getBiome(pos)).contains(Type.SNOWY));
		}
	}
