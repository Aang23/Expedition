...
public class CaveListWrapper {
	...
	public double getCenterScore(){
		...
		for (CavePosition checkpos : this.cavelist){
			...
			if (posFailScore < bestscore){
				...
				if (checkpos.ceiling-checkpos.floor > (bestpos != null ? bestpos.ceiling-bestpos.floor : 0) ){
					bestpos = checkpos;
				}
				else if (checkpos.ceiling-checkpos.floor == (bestpos != null ? bestpos.ceiling-bestpos.floor : 0)){
	...
	private int airHash = Material.AIR.hashCode();
