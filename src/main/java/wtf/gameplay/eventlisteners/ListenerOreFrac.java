...
public class ListenerOreFrac {
	...
	{
		if (!event.getEntityPlayer().capabilities.isCreativeMode && event.getItemStack() != ItemStack.EMPTY){
		...
		if (!event.getPlayer().capabilities.isCreativeMode && !silk){
			...
			if (BlockSets.oreAndFractures.contains(block))
			{
				if (GameplayConfig.fracSimple){
					...
					int toolLevel = tool == ItemStack.EMPTY ? 0 : tool.getItem().getHarvestLevel(tool, "pickaxe", event.getPlayer(), event.getState());
