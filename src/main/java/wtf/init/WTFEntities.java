...
public class WTFEntities {
	...
	public static void registerEntity(Class<? extends Entity> entityClass, String name){
		registerEntity(new ResourceLocation(Core.coreID, name), entityClass, name);
	}

	public static void registerEntity(ResourceLocation resourceLocation, Class<? extends Entity> entityClass, String name)
	{
		int entityId = counter;
		counter++;
		EntityRegistry.registerModEntity(resourceLocation, entityClass, name, entityId, Core.instance, 64, 1, true, counter*31, counter*62);
		//EntityRegistry.registerEgg(EntityList.getKey(entityClass), counter*31, counter*62);
	}
