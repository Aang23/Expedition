...
public class BlockDenseOreFalling extends AbstractBlockDerivativeFalling {
    ...
    public void harvestBlock(World worldIn, EntityPlayer player, BlockPos pos, IBlockState state, @Nullable TileEntity te, @Nullable ItemStack stack)
    {
        ...
        if (this.canSilkHarvest(worldIn, pos, state, player) && stack != null && EnchantmentHelper.getEnchantmentLevel(Enchantments.SILK_TOUCH, stack) > 0)
        {
            java.util.List<ItemStack> items = new java.util.ArrayList<>();
            ItemStack itemstack = new ItemStack(this.parentForeground.getBlock(),1,state.getBlock().getMetaFromState(state));
