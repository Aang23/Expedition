...
public class BlockDecoAnim extends AbstractBlockDerivative{
	...
	public BlockDecoAnim(IBlockState state) {
		super(state, state);
	...
	public enum ANIMTYPE implements IStringSerializable {
		...
		ANIMTYPE(int ID, String name, BlockSets.Modifier mod) {
			this.ID = ID;
			this.name = name;
			this.modifier = mod;
		}
	...
	public IBlockState getStateFromMeta(int meta)
	{
		...
		return this.getDefaultState().withProperty(TYPE, ANIMTYPE.values()[meta]).withProperty(FAST, fast);
	}
	...
	{
		return new BlockStateContainer(this, TYPE, FAST);
	}
	...
	public boolean canRenderInLayer(IBlockState state, BlockRenderLayer layer)
    {
		...
		case SOLID:
			return true;
		case TRANSLUCENT:
			return state.getValue(TYPE) == ANIMTYPE.LAVA_CRUST;
		default:
			break;
